...
日期：2026-08-07
更新：2026-08-08
状态：Draft，已锁定总体边界，细节通过 Wayfinder 决策票据继续收敛
...
| 图片生成 | Pi Images API registry | 扩展自有 Images API Provider；当前上游仅内置 OpenRouter | Codex OAuth 通道必须先通过真实账户兼容性 spike |
...
| Subagent 基础 | Pi 官方 `examples/extensions/subagent` | 可作为最小语义基线 | 不直接把示例代码当生产组件 |
| Subagent 执行 | `@agwab/pi-subagent` | 优先评估 | 取消、并发、事件、许可证和打包通过 |
...
| MCP | Pi Extension/Package 生态 | `[待研究]` | 必须覆盖 transport、认证、重连、日志、工具启停和权限 |
...
| 图片生成 | 必须验证 ChatGPT/Codex OAuth 凭据通道；若没有受支持接口则禁用并明确提示，但不阻断其他能力发布；不得用 API key/OpenRouter 静默替代 |
| Slides 整页生成 | 保留；复用现有 HTML→PPTX；PPTX 可打开、文字可编辑、无越界/重叠、图片完整，失败时保留原页 |
| PDF→DOCX | MinerU 作为默认关闭的 OCR 服务商，可直接请求 `docx` 输出；Pandoc 不进入主链路 |
...
MinerU 官方 API 已支持在精确解析任务中通过 `extra_formats: ["docx"]` 请求 DOCX，
...
2. 桌面端使用轮询，不依赖公网 callback；处理 `pending`、`running`、`converting`、
   `done` 和 `failed` 状态；
3. 完成后从 `full_zip_url` 下载结果包，只提取并校验目标 DOCX；
...
- WebDAV 支持 HTTPS、自定义根路径和常用 Basic/Digest/Bearer 认证组合；
- S3 支持 AWS S3 与 S3-compatible endpoint、region、bucket、prefix 和 path-style；
...
- 检测到远端分叉时保持本地工作副本为 current revision，把远端 revision 保存为
  Conflict Copy，不静默覆盖；
- 用户选择远端版本后，把该选择写为新的本地 current revision，并将被替换的旧版本
  转为 Conflict Copy；不得用跨设备 wall-clock 单独决定胜者。

## 9. 迁移工作包

### WP0：依赖与契约基线

- 固定同一版本的 `pi-agent-core`、`pi-ai`、`pi-coding-agent`；不得混用 Pi 版本。
- 将 Node/CI 下限提升到 Pi 当前要求。
...
- 正式 IPC 在 macOS/Linux 使用 Unix Domain Socket，在 Windows 使用 Named Pipe，
  每次启动生成一次性握手 token；stdio 仅用于独立 debug/test。
...
- 完成 Codex OAuth 图片生成兼容性 spike；没有受支持接口时禁用功能并明确提示，不能静默改用另一计费通道。
...
- 增加默认关闭的 OCR 服务商设置，以 MinerU `docx` 输出替换 Genspark PDF→DOCX，并完成轮询、下载与清理。
...
| AR-007 | 文档绑定 | 多文档、多 Session 集成测试 | 每条 Session 只解析并操作绑定文档，不能隐式切换到另一文档 |
| AR-008 | 旧数据清除 | 升级 fixture | 旧聊天、索引、Provider 配置、明文 key 和 Genspark token 均删除且不迁移 |
| AR-009 | Sidecar 生命周期 | Electron + 独立 debug E2E | Runtime 随包交付，由 Electron 启停/恢复/回收，同一入口可独立调试且不是 `utilityProcess` |
| AR-010 | 本地 IPC | 三平台打包 E2E | UDS/Named Pipe 完成 token 与版本握手；拒绝旧 token、错误版本和非父实例连接 |
...
| MD-005 | Codex OAuth 图片 | 发行前真实账户兼容性测试 | 有受支持接口时可生成；无接口时功能禁用并解释原因，且不调用内部接口或替代计费通道 |
...
| OCR-003 | MinerU 直出 DOCX | 官方 API sandbox/fixture 集成测试 | 开启后本地 PDF 经签名上传、异步解析和下载得到可打开 DOCX，无 Pandoc 依赖 |
...
| SY-001 | WebDAV | Provider contract + 双客户端 E2E | 首次上传、增量同步、远端拉取、删除保护、重试和进度通过 |
| SY-002 | S3 | AWS S3/MinIO contract + 双客户端 E2E | region/endpoint/bucket/prefix/path-style 可配，AWS 与 S3-compatible 行为一致 |
...
| PK-001 | Electron 打包 | macOS/Windows/Linux smoke | Pi ESM、Extension、MCP stdio 和资源路径在安装包中正常工作 |
| QA-001 | 自动化质量 | CI | 新平台模块覆盖率 ≥95%，全仓 typecheck、lint、license、notices 通过 |

## 11. 当前待验证

这些问题已进入 Wayfinder frontier，必须用研究或原型取得证据后再关闭：

1. Codex OAuth 图片生成是否存在可公开依赖的稳定接口。
2. Pi MCP 生态中应固定哪个实现，哪些能力需要自有适配层。
3. Pi Subagent 生态中应固定哪个实现，能否满足取消、事件、预算和 lineage 契约。
4. Sidecar 在 macOS/Windows/Linux 的可执行文件封装与 Pi ESM/Extension 打包方式。
5. MinerU 对代表性 PDF 的 DOCX 版式保真基线与不可接受降级边界。
6. WebDAV/S3 共用 manifest、revision 和 Conflict Copy 的最小可行数据模型。
