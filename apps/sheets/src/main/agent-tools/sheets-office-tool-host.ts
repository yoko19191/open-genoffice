...
import type { SheetsOfficeToolRendererClient } from './renderer-client'

type RendererClient = Pick<SheetsOfficeToolRendererClient, 'request'>

export interface SheetsOfficeToolHostOptions {
...
function descriptor(toolId: string): OfficeToolDescriptor | undefined {
  const definition = SHEETS_OFFICE_TOOL_DEFINITIONS.find(({ id }) => id === toolId)
...
function imageArtifactIds(input: unknown): string[] {
  ...
  return [
    ...new Set(
      operations.flatMap((operation) => {
        ...
        const candidate = operation as { op?: unknown; artifactId?: unknown }
        return candidate.op === 'add_image' && typeof candidate.artifactId === 'string'
...
export class SheetsOfficeToolHost {
  ...
  invoke(request: OfficeToolInvocation): Promise<OfficeToolReceipt> {
    ...
    try {
      parseSheetsOfficeToolInput(request.toolId, request.input)
    } catch {
  ...
  private async captureSnapshot(
    ...
    const snapshotId = response.result.snapshot.token
  ...
  private async restoreSnapshot(input: {
    ...
    if (!response.ok || response.result.kind !== 'restored') {
      throw new Error(response.ok ? 'executor_unavailable' : response.errorCode)
    }
  ...
  private async executeUnaborted(
    ...
    const images: SheetsOfficeImagePayload[] = []
    try {
      for (const artifactId of imageArtifactIds(request.input)) {
    ...
    const snapshot =
      boundary?.kind === 'snapshot' ? this.snapshots.get(boundary.snapshotId)?.snapshot : undefined
