...
function phaseLabel(
  ...
  labels: { thinking: string; replying: string; working: string },
): string {
  if (projection?.tools.some((tool) => tool.state === 'requested' || tool.state === 'running')) {
    return labels.working
  }
  return projection?.thinking?.streaming ? labels.thinking : labels.replying
}

export function AiPanel({ onCollapse }: { onCollapse: () => void }): ReactElement {
  const { t } = useI18n()
  ...
  const typingLabel = phaseLabel(projection, {
    thinking: t('aiThinking'),
    replying: t('aiReplying'),
    working: t('aiWorking'),
  })
...
function ToolStatusList({ tools }: { tools: AgentPanelTool[] }): ReactElement {
  return (
