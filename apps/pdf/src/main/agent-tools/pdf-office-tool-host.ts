...
import type { PdfOfficeContextSnapshot, PdfOfficeEditSnapshot } from '../../shared/ipc'
import type { PdfOfficeToolRendererClient } from './renderer-client'

type RendererClient = Pick<PdfOfficeToolRendererClient, 'request'>

export interface PdfOfficeToolHostOptions {
...
function descriptor(toolId: string): OfficeToolDescriptor | undefined {
  const definition = PDF_OFFICE_TOOL_DEFINITIONS.find(({ id }) => id === toolId)
...
export class PdfOfficeToolHost {
  ...
  invoke(request: OfficeToolInvocation): Promise<OfficeToolReceipt> {
    ...
    try {
      parsePdfOfficeToolInput(request.toolId, request.input)
    } catch {
