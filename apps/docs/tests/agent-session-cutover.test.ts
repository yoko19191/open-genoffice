...
describe('Docs Agent Session cutover', () => {
  it('uses the shared Session controller and the narrow Office Tool bridge', async () => {
    ...
    expect(panel).toContain('.prompt(instruction, artifacts)')
    ...
    expect(preload).toContain("contextBridge.exposeInMainWorld('agentArtifacts'")
    ...
    expect(rendererTypes).toContain('agentArtifacts: DocsAgentArtifactsApi')
