...
import type {
  SlidesOfficeEditSnapshot,
  SlidesOfficeImagePayload,
} from '../../shared/slides-office-tools'
import type { SlidesOfficeToolRendererClient } from './renderer-client'

type RendererClient = Pick<SlidesOfficeToolRendererClient, 'request'>

export interface SlidesOfficeToolHostOptions {
...
function descriptor(toolId: string): OfficeToolDescriptor | undefined {
  const definition = SLIDES_OFFICE_TOOL_DEFINITIONS.find(({ id }) => id === toolId)
...
export class SlidesOfficeToolHost {
  ...
  constructor(private readonly options: SlidesOfficeToolHostOptions) {
  ...
  invoke(request: OfficeToolInvocation): Promise<OfficeToolReceipt> {
    ...
    try {
      parseSlidesOfficeToolInput(request.toolId, request.input)
    } catch {
  ...
  private async captureSnapshot(
    ...
    if (!response.ok || response.result.kind !== 'snapshot') throw new Error('executor_unavailable')
  ...
  private async executeUnaborted(
    ...
    const images: SlidesOfficeImagePayload[] = []
    try {
      for (const artifactId of artifactIds(request.input)) {
